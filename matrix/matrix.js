...
(function (windows) {
    ...
    // 生成相同数字组成的数组
    Matrix.generateArrBySameValue = function(value, len) {
        let arr = [];
        for (let i = 0; i < len; i++) {
            arr.push(value);
        }
    ...
    // 计算向量的内积
    Matrix.innerProduct = function(matrix1, matrix2) {
        if(matrix1.w === matrix2.w && matrix1.w === 1) {
            let result = 0;
            matrix1.mat.forEach(([x], i) => {
                let y = matrix2.mat[i][0];
                result += x*y;
            });
            return result;
        }
    ...
    // 计算2个向量是否正交
    Matrix.orthogonal = function (matrix1, matrix2) {
        const r = Matrix.innerProduct(matrix1, matrix2);
        return r === 0;
    };

    // 向量空间
    windows.VectorSpace = function (vectors) {
        let isVector = true;
        let dimension,vectorSpaceDimension;
        if (vectors.forEach && vectors.length > 0) {
            vectors.find(vector => {
                if (vector instanceof Matrix && vector.w === 1) {
                    if (!dimension) {
                        dimension = vector.h;
                        let isZero = true;
                        vector.mat.find(x => {
        ...
        if (!isVector) {
            throw new Error('向量空间必须依赖于Matrix且每个向量必须是同样的宽高,且向量不能为零向量');
        }
        vectorSpaceDimension = vectors.length;
